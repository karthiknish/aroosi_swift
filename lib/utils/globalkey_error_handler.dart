...
class GlobalKeyErrorHandler {
  static final GlobalKeyErrorHandler _instance = GlobalKeyErrorHandler._internal();
  ...
  void init() {
    ...
    FlutterError.onError = (FlutterErrorDetails details) {
      ...
      if (details.exception.toString().contains('Multiple widgets used the same GlobalKey') ||
        ...
        if (details.stack != null) {
          ...
          final keyMatch = RegExp(r'GlobalKey<([#\w]+)>').firstMatch(stackString);
    ...
    PlatformDispatcher.instance.onError = (error, stack) {
      if (error.toString().contains('GlobalKey') ||
          error.toString().contains('Multiple widgets used the same GlobalKey')) {
